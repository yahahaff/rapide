...
// Incr 方法封装
func (rds RedisClient) Incr(key string) error {
	return rds.Client.Incr(rds.Context, key).Err()
}

// Expire 方法封装
func (rds RedisClient) Expire(key string, expiration time.Duration) error {
	return rds.Client.Expire(rds.Context, key, expiration).Err()
}
