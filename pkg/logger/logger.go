...
// getLogWriter 日志记录介质。Rapide 中使用了两种介质，os.Stdout 和文件
func getLogWriter(filename string, maxSize, maxBackup, maxAge int, compress bool, logType string) zapcore.WriteSyncer {
	// 如果配置了按照日期记录日志文件
	if logType == "daily" {
		logname := time.Now().Format("2006-01-02.log")
		filename = strings.ReplaceAll(filename, "rapide.log", logname)
		...
		lumberJackLogger := &lumberjack.Logger{
			...
			MaxAge:     maxAge,
			Compress:   compress,
			LocalTime:  true, // 使用本地时间进行日志轮转
		}
		return zapcore.AddSync(lumberJackLogger)
	}

	// 将日志文件写入单一文件，利于日志排查，并且使用linux logrotate进行按日期轮转
...
//// ErrIf 当 err != nil 时记录 error 等级的日志并抛出异常
//func ErrIf(err error) {
//	if err != nil {
//		// 记录并返回错误信息 使用panic触发全局错误,middlewares.ErrorHandler()返回500错误
