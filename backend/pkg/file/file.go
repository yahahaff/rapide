...
import (
	...
	"github.com/yahahaff/rapide/backend/pkg/app"
	"github.com/yahahaff/rapide/backend/pkg/helpers"
	"mime/multipart"
	"os"
	"path/filepath"
	"strings"
...
// SaveUploadAvatar 修改头像验证器 调用
func SaveUploadAvatar(c *gin.Context, file *multipart.FileHeader) (string, error) {

	var avatar string
	// 确保目录存在，不存在创建
	publicPath := "public"
	dirName := fmt.Sprintf("/uploads/avatars/%s/%s/", app.TimenowInTimezone().Format("2006/01/02"), service.Entrance.SysService.AuthService.CurrentUID(c))
