...
// Attempt 尝试登录
func (as *AuthService) Attempt(loginId string, password string) (sys.User, error) {
	userModel := sysDao.GetByMulti(loginId)
	if userModel.ID == 0 {
...
func (as *AuthService) CurrentUser(c *gin.Context) sys.User {
	...
	// 预加载角色关联
	database.DB.Preload("Role").Preload("Dept").Find(&userModel)
